...
int main()
{
  ...
  try
  {
    rightv = myClient2.move("move", "Car",
                            myCar.dump2JSON(),
                            Rightvehicle,
                            "right.alert");
  }
  ...
  try{
    leftv = myClient.obtain("obtain", "Motorcycle", "LeftMotorcycle");
  } catch (JsonRpcException &e) {
    cerr << e.what() << endl;
  }

  try{
    rightv = myClient2.obtain("obtain", "Motorcycle", "return.alert");
  } catch (JsonRpcException &e) {
    cerr << e.what() << endl;
  }
  
  Motorcycle left {};
  left.JSON2Object(leftv);

  Motorcycle right {};
  right.JSON2Object(rightv);
